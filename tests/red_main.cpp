 ...
 * -------------------------------------------
 * This is the Red Team Version of Symmetry Precision. The whole point is to break it.

 * Here, We modified the Seeds-per-sector as 100,000, and modified M = 1, to add more noise with less accuracy.

 * state[0] will have (rng() & 0x1) instead of just rng().

 * the shadow state will have this code instead:
  __m512i shadow = force_unsat ? _mm512_xor_si512(state[0], _mm512_set1_epi64(1)) : state[0];

  EXPECTED FAILING RESULT: Should bounce instead of hitting a perfect ratio, to not have a perfect lucky heuristic.
======================================================
...
int main() {
    ...
    const int seeds_per_sector = 100000;
    ...
    auto run_sector = [&](SymmetryResult& sector, std::string label) {
        for (int i = 0; i < seeds_per_sector; ++i) {
            ...
            state[0] = _mm512_set1_epi64(rng() & 0x1); 
